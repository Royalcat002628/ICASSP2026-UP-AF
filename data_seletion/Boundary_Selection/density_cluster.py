...
def selection(class2index_list, class_core_features, features, indices, nums, budget,
    ...
    for class_idx in range(nums):
        ...
        cur_class_nums    = len(cur_class_indices)
        # —— 如果该类根本没有样本，跳过 ——
        if cur_class_nums == 0:
            continue
        # 2) 计算本类预算，不要超过现有样本数 ——
        orig_budget = allocations[class_idx]
        class_budget = min(orig_budget, cur_class_nums)
        ...
        # 3) 定位核心样本在本类中的局部下标
        cur_class_list = []
        for i, idx in enumerate(cur_class_indices):
            if idx == indices[class_idx]:
                cur_class_list = [i]
                break
        # （万一没找到，就用第一个）
        if not cur_class_list:
            cur_class_list = [0]
        # invalid_number_per_node = int(cur_class_nums / class_budget)
        invalid_number_per_node = max(1, cur_class_nums // class_budget)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--feature_path', default='../subsets/MIT_Place_Pulse', type=str)
    ...
    args.output_dir = f"{args.feature_path}/Density_Cluster/" if args.output_dir is None else args.output_dir
